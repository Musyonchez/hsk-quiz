...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Only the receiver of a request may ignore it.
  if (row.friendId !== user.id) {
    return NextResponse.json({ error: "Not your request to ignore." }, { status: 403 });
  }
  if (row.status !== "pending") {
    return NextResponse.json({ error: "Request already resolved." }, { status: 400 });
  }

  await prisma.friendship.update({ where: { id: requestId }, data: { status: "ignored" } });
